...
\end{aligned}
$$
Reading from top to bottom, this is a *generative recipe*: first draw the rates and the switchpoint, then construct the piecewise-constant rate vector, then draw the observed counts. Note that $\tau$ is **discrete**, which has consequences for inference. We will need a sampler that handles the mix of discrete and continuous parameters (@sec-inference).
