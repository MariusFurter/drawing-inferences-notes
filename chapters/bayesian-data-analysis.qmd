...
Each sampling statement draws a value from a specified kernel. Reading from top to bottom, this is a *generative recipe*: first draw the rates and the switchpoint, then construct the piecewise-constant rate vector, then draw the observed counts. Note that $\tau$ is **discrete**, which has consequences for inference. We will need a sampler that handles the mix of discrete and continuous parameters (@sec-inference).
...
To inspect the results, we convert the MCMC output to an ArviZ `InferenceData` object and call `az.summary`:
...
posterior = idata_fake["posterior"].to_dataset()
...
Having verified the pipeline on fake data, we now condition on the real observations. Each call to `mcmc.run()` replaces the previous results. This is why we saved the fake-data output as `idata_fake` above. Now we run on the actual coal mining data:
...
We convert the MCMC output to an ArviZ `InferenceData` object and call `az.summary`:

```{python}
#| label: mcmc-summary
#| code-fold: false
idata = az.from_numpyro(mcmc)
az.summary(idata, var_names=["rho_1", "rho_2", "tau"])
```
...
#| code-fold: true
az.plot_trace(
    idata, var_names=["rho_1", "rho_2", "tau"], figure_kwargs={"figsize": (7, 2)}
)
plt.show()
```
...
#| code-fold: true
az.plot_rank(idata, var_names=["rho_1", "rho_2", "tau"], figure_kwargs={"figsize": (7, 2)})
plt.show()
```
...
#| code-fold: false
az.psense_summary(idata, var_names=["rho_1", "rho_2", "tau"])
```
...
def model_sigmoid(n_years, counts=None):
    ...
    idx = jnp.arange(n_years, dtype=jnp.float32)
    weight = jax.nn.sigmoid(-(idx - tau) / s)
...
#| code-fold: false
idata_sig = az.from_numpyro(mcmc_sig, log_likelihood=True)
az.summary(idata_sig, var_names=["rho_1", "rho_2", "tau", "s"])
```
...
With all three models fit, we extract or construct their `InferenceData` objects with pointwise log-likelihoods and pass them to `az.compare`:

```{python}
#| label: compute-loo
#| code-fold: false
idata_const = az.from_numpyro(mcmc_const, log_likelihood=True)
# idata already has log_likelihood from the DataTree built in the sensitivity section
idata_cp = idata

comparison = az.compare(
    {"Constant rate": idata_const, "Discrete change point": idata_cp, "Sigmoid change point": idata_sig}
)
comparison
```
...
#| code-fold: true
loo_cp = az.loo(idata_cp, var_name="obs")
az.plot_khat(loo_cp)
plt.tight_layout()
plt.show()
```
