...
Let $g :  \kernel U \otimes V \otimes W$ be a kernel with no inputs. We say that $U$ and $V$ are **conditionally independent given $W$**, denoted $(U \perp V \mid W)$, if $g$ can be written in the following form:
$$
g(u, v, w) = a(u \mid w) \, b(v \mid w) \, c(w),
$$
...
The above definition can be extended to kernels $g : X \kernel U \otimes V \otimes W$ with inputs: $(U \perp V \mid W)$ if $g$ can be written in the form
$$
g(u, v, w \mid x) = a(u \mid w) \, b(v \mid w,x) \, c(w \mid x).
$$
...
**Using importance sampling for a conditioned model.** Now consider the situation after conditioning. Pre-conditioning a kernel $g : X \kernel Y \otimes Z$ on $Z = \bar z$ gives the unnormalized kernel $g_{\mid Z = \bar z}(y \mid x) = g(y, \bar z \mid x)$. The normalized conditional is $g(y \mid \bar z, x) := \frac{g_{\mid Z = \bar z}(y \mid x)}{\sum_{y'} g_{\mid Z = \bar z}(y' \mid x)}$. Hence, the conditional expectation of a test function $t$ is:
$$
...
$$
\mathbb{E}_{g(y \mid \bar z, x)}[ \, t(y) \,]  \approx \frac{\sum_{i=1}^N w(y_i, x) \, t(y_i)}{\sum_{i=1}^N w(y_i, x)}.
$$
