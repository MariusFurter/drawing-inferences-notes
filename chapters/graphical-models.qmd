...
How do we compute the composite kernel $(g \then f)$ from $f$ and $g$? If we interpret $f$ and $g$ as conditional distributions, the chain rule and conditional independence give:
...
If $f$ is a probability kernel, this sum equals 1, so deleting the output is the same as deleting the input. This property characterizes probability kernels — we call them **normalized**.
...
where the subscripts denote what wire the variable corresponds to. In general, any string diagram of probability kernels evaluates to an expression of this form:
$$
...
Each composition rule translates directly into a tensor operation. We represent a kernel $f : X \kernel Y$ as a tensor `f` of shape `(|X|, |Y|)`, where `f[x, y]` stores $f(y \mid x)$. Since our variables take values $0, 1, \ldots, n-1$, we can use them directly as indices.

[Issue: The results no longer have the shape (|Input|, |Output|) after composition?]

- **Sequential composition** sums over the shared variable: $(f \then g)(z \mid x) = \sum_y f(y \mid x) \, g(z \mid y)$.

    ```python
    # f has shape (|X|, |Y|), g has shape (|Y|, |Z|)
    fg = torch.einsum('xy,yz->xz', f, g)
    ```

- **Parallel composition** takes the outer product: $(f_1 \otimes f_2)(y_1, y_2 \mid x_1, x_2) = f_1(y_1 \mid x_1) \, f_2(y_2 \mid x_2)$.

    ```python
    # f1 has shape (|X1|, |Y1|), f2 has shape (|X2|, |Y2|)
    f1f2 = torch.einsum('ab,cd->acbd', f1, f2)
    ```

- **Copy composition** multiplies tensors that share an index without summing it out: the result at $(x, y, z)$ is $f(y \mid x) \, g(z \mid y)$.

    ```python
    # f has shape (|X|, |Y|), g has shape (|Y|, |Z|)
    fg = torch.einsum('xy,yz->xyz', f, g)
    ```

- **Delete** sums out the corresponding index.

    ```python
    # marginalize out the last axis
    result = tensor.sum(dim=-1)
    ```

We can therefore evaluate any string diagram by performing a sequence of tensor operations. The order in which we perform these operations does not affect the final result, but it can dramatically affect the computational cost.

[Issue: Compute computational cost more carefully.]
Consider the fire alarm model with binary variables ($|F| = |S| = |L| = |A| = 2$). The naive sum-product form
$$
\sum_{x_f, x_l} f(x_f \mid \bullet) \, s(x_s \mid x_f) \, p(x_l \mid x_f) \, t(x_a \mid x_s, x_l)
$$
requires us to evaluate the product for every combination of $x_f$ and $x_l$ — that is $2 \times 2 = 4$ terms for each pair $(x_s, x_a)$. But we can reduce the work by pushing sums inward. Summing over $x_l$ first gives:
$$
\sum_{x_f} f(x_f \mid \bullet) \, s(x_s \mid x_f) \, \sum_{x_l} p(x_l \mid x_f) \, t(x_a \mid x_s, x_l).
$$

Here the inner sum over $x_l$ only involves two terms (for each fixed $x_f$), and the outer sum over $x_f$ also involves two terms. We can rearrange further:
$$
\sum_{x_l} t(x_a \mid x_s, x_l) \, \sum_{x_f} f(x_f \mid \bullet) \, s(x_s \mid x_f) \, p(x_l \mid x_f).
$$

Each rearrangement corresponds to a different order of tensor contractions, and the best order depends on the structure of the diagram. Finding the optimal contraction order is NP-hard in general, but good heuristics exist. This is the basis for exact inference algorithms like *variable elimination*. [Cite Darwiche for more information.]

## Sampling from models

In the [Introduction](intro.qmd), we saw how to sample from a generative model by implementing each box in the string diagram as a random function. But why does this work? In this section, we describe the general procedure and explain why it is sound.
...
There are several well-known frameworks for representing probabilistic models, each with its own strengths. Here we compare string diagrams to the three common alternatives.

### Bayesian networks

A **Bayesian network** represents a joint distribution over a set of random variables using a directed acyclic graph (DAG). Each node corresponds to a variable, and each node is equipped with a conditional probability distribution (CPD) that specifies the distribution of that variable given its parents. The joint distribution factors as a product of these CPDs:
$$
...
[Advantage is that drawing DAG is somewhat simpler than drawing a string diagram (because we usually think in terms of variables rather than mechanisms and because DAGs are less informative).]
...
Beyond expressiveness, there are two deeper issues with Bayesian networks.

First, DAGs are **not compositional**. You cannot take a subgraph and treat it as a single node in a larger network. In string diagrams, any sub-diagram can be wrapped into a box, producing a new kernel that can be plugged into other diagrams. This compositional structure makes it easy to build complex models from reusable components.

[Additional point: String diagrams can enforce typing of variables, and capture patterns of reuse (e.g. same f can appear in several places).]
