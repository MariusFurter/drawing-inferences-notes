...
We assign every subset $E \subseteq \Omega$ a number $p(E)$ that captures how likely we think $E$ is. If our assignment satisfies the following two properties, we call it a **probability measure** on $\Omega$:
...
We will now describe a convenient way to build and reason about generative models using **string diagrams**. This will allow us to visually manipulate complex models without having to resort to tedious algebraic calculations. Two examples of string diagrams are shown below. The one on the left is represents a typical causal model. The one on the right illustrates other syntactic possibilities.
...
String diagrams encode the structure of our model and make is easy to see which variables depend on each other. We will see in the next chapter how to read off conditional independence assumptions from the diagram, and how to use it to organize calculations.
...
It says that of we transform an input by $f$ and then delete the results, it's the same as directly deleting the input. Most boxes we will encounter are normalized and hence satisfy this property.

We will see more examples of ways to calculate with string diagrams in the next chapter. For now, we can use these three to demonstrate calculate what happens to the fire alarm diagram when we delete the bottom output and simplify:
...
print(
    f"At least one twin affected: {at_least_one_twin_affected / III3_unaffected:.4f}",
    f"\nBoth twins affected: {both_twins_affected / III3_unaffected:.4f}",
)
```
...
To leverage modern hardware like GPUs, probabilistic programming often relies on **vectorized operations**. Instead of running the simulation in a loop $N$ times, we generate $N$ samples in parallel using tensors. We encode genotypes as binary values: 0 for unaffected ($XX$ or $XY$) and 1 for carrier or affected ($xX$ or $xY$). Here is the same model implemented using PyTorch:
...
### WeightedSampling.torch implementation
We conclude by demonstrating how this problem can be implemented using the `WeightedSampling.torch` library. 
