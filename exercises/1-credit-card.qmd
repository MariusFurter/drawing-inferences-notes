...
If you prefer not to use PyTorch, you can use `if`/`else` with [`random.binomialvariate`](https://docs.python.org/3/library/random.html#random.binomialvariate) from base Python. The logic is the same, but you will need a loop to generate multiple samples, which will be much slower.
:::

::: {.callout-tip collapse="true" title="Solution"}

We write a generative function for each box, using arithmetic to compute the conditional probability before sampling with [`torch.distributions.Bernoulli`](https://pytorch.org/docs/stable/distributions.html#torch.distributions.Bernoulli). Since the inputs are 0/1 values, we can express the conditional probabilities as arithmetic expressions that work on both scalars and tensors. To sample a single value, call with `N=1`; to sample a batch of `N` values at once, pass a larger `N`. Alternatively, you could use `if`/`else` with [`random.binomialvariate`](https://docs.python.org/3/library/random.html#random.binomialvariate) from base Python — the logic is the same, just less compact and not vectorized.

**Generators $f$ and $t$.** We choose a fraud rate of 0.088% based on [this report](https://www.ecb.europa.eu/pub/pdf/cardfraud/ecb.cardfraudreport202110~cac4c418e8.en.pdf) by the European Central Bank. The same report puts the cross-border rate at 11%.
...
print(f"P(F=1 | L=1, U=1, H=0, A=0) = {est:.4f}, acceptance rate = {rate:.2e}")
```
...
print(f"P(F=1 | L=1, U=1, H=0, A=0, T=1) = {est:.4f}, acceptance rate = {rate:.2e}")
```
...
print(f"P(F=1 | L=1, U=1, H=1, A=1, T=0) = {est:.4f}, acceptance rate = {rate:.2e}")
```
