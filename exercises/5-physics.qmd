...
We plot 10 randomly selected trajectories from the prior:
...
We plot histograms of the posterior distributions of flight time and maximum altitude:
...
2. Simulates the trajectory using `simulate_gravity` with launch from origin, planet at $(5, 3)$ with `planet_mass = 30`.

Run the model with `num_particles=5000` and use `summary()` to inspect the result.

**Part 2.** Select 10 random particles from the result and plot their trajectories on a single figure. Mark the planet's position with a large dot and Station Alpha at the origin.

**Part 3.** Using the sampled velocities, re-run `simulate_gravity` to obtain all endpoints. Plot a **scatter** of the prior distribution of endpoints (final $(x_T, y_T)$ for all particles). Where is the density highest? How does the planet's gravity shape the distribution?
...
2. Write a function decorated with `@model`. Inside, use `sample("vx", dist.Normal(0.0, 3.0))` and similarly for `vy`, then call `simulate_gravity`.
3. Call the model with `num_particles=5000`:
   ```python
...
2. Pick 10 random indices: `idx = torch.randperm(len(vx_samples))[:10]`.
3. Re-run `simulate_gravity` for the selected particles and plot each trajectory.

**Part 3 — scatter of endpoints:**
...
```{python}
# | code-fold: false
# Part 2: Re-simulate 10 random trajectories
vx_samples = result_probe["vx"]
vy_samples = result_probe["vy"]
idx = torch.randperm(len(vx_samples))[:10]
...
```{python}
# | code-fold: false
# Part 3: Re-simulate all particles and extract endpoints
all_xs, all_ys = simulate_gravity(
    x0=0.0, y0=0.0,
    vx0=vx_samples, vy0=vy_samples,
    planet_x=planet_x, planet_y=planet_y,
    planet_mass=planet_mass,
    dt=0.02, n_steps=300,
)
...
**Part 1.** Modify your model to condition on the observed endpoint. Since the observation is two-dimensional, combine both coordinates into a **single** `observe` call using a multivariate normal:
```python
endpoint = torch.stack([xs[-1], ys[-1]], dim=-1)
...
Combine the $x$ and $y$ observations into one `observe` call:
...
We overlay 500 resampled posterior trajectories:
...
all_post_xs, all_post_ys = simulate_gravity(
    x0=0.0, y0=0.0,
    vx0=vx_resampled, vy0=vy_resampled,
    planet_x=planet_x, planet_y=planet_y,
    planet_mass=planet_mass,
    dt=0.02, n_steps=300,
)
...
dt_sim = 0.02
vx_traj = (all_post_xs[1:] - all_post_xs[:-1]) / dt_sim
vy_traj = (all_post_ys[1:] - all_post_ys[:-1]) / dt_sim
speed = torch.sqrt(vx_traj**2 + vy_traj**2)
v_max = speed.max(dim=0).values
...
These derived quantities are obtained by plugging posterior velocity samples into the simulation and computing functions of the resulting trajectories. Each sample gives one value, so the collection represents the full posterior distribution over that quantity — not just a point estimate. The closest-approach distribution tells us how likely the probe was to have had a dangerous flyby, while the maximum-speed distribution reflects how much gravitational acceleration it experienced.
:::