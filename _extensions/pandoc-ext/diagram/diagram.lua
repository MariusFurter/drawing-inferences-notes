  ...
  compile = function (self, src, user_opts)
    return with_temporary_directory("tikz", function (tmpdir)
      return with_working_directory(tmpdir, function ()
        ...
        -- Build the TeX file directly to preserve raw LaTeX in additional-packages
        -- We need to extract raw LaTeX from Pandoc Blocks without using
        -- stringify (which strips commands) or pandoc.write (which adds document wrapper).
        local function blocks_to_raw_latex(blocks)
          local parts = {}
          for _, block in ipairs(blocks) do
            ...
            elseif block.t == 'Para' or block.t == 'Plain' then
              for _, inline in ipairs(block.content) do
                if inline.t == 'RawInline' and (inline.format == 'latex' or inline.format == 'tex') then
        ...
        local additional = ''
        if user_opts['additional-packages'] then
          local ap = user_opts['additional-packages']
          local atype = pandoc.utils.type(ap)
          if type(ap) == 'string' then
            additional = ap
          elseif atype == 'Blocks' then
            additional = blocks_to_raw_latex(ap)
          else
            additional = stringify(ap)
          end
        end
        local header_includes = ''
        if user_opts['header-includes'] then
          local hi = user_opts['header-includes']
          local hitype = pandoc.utils.type(hi)
          if type(hi) == 'string' then
            header_includes = hi
          elseif hitype == 'Blocks' then
            header_includes = blocks_to_raw_latex(hi)
          elseif hitype == 'List' then
            for _, item in ipairs(hi) do
              local itype = pandoc.utils.type(item)
              if itype == 'Blocks' then
                header_includes = header_includes .. blocks_to_raw_latex(item) .. '\n'
              else
                header_includes = header_includes .. stringify(item) .. '\n'
              end
            end
          else
...
local function configure (meta, format_name)
  ...
  -- diagrams-path: base directory for .tikz file references
  local diagrams_path = conf['diagrams-path']
    and stringify(conf['diagrams-path'])
    or nil
...
local pdf2svg = function (imgdata)
  ...
  local svg_file = 'diagram-' .. pandoc.utils.sha1(imgdata) .. '.svg'
  write_file(pdf_file, imgdata)
  pandoc.pipe('pdf2svg', {pdf_file, svg_file}, '')
  local svg_fh = io.open(svg_file, 'rb')
...
local function code_to_figure (conf)
  return function (block)
    ...
    -- If a 'file' attribute is present, load the .tikz file content.
    -- The code block text (%%| directives) is prepended to the file content.
    if block.attributes.file then
      local filepath = block.attributes.file
      -- Resolve relative to diagrams-path if configured
      if conf.diagrams_path and not filepath:match('^/') then
        filepath = pandoc.path.join({conf.diagrams_path, filepath})
      end
      local ok, file_content = pcall(read_file, filepath)
      if not ok then
        warn('diagram: could not read file: ' .. filepath)
        return nil
      end
      -- Prepend any %%| directives from the code block to the file content
      local directives = block.text:match('^(%%.-\n)') or ''
      if block.text:match('^%%') then
        block.text = block.text .. '\n' .. file_content
      else
        block.text = file_content
      end
      block.attributes.file = nil  -- remove so it doesn't interfere downstream
    end
